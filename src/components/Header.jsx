...
window.Components.Header = ({
    ...
    const {
        Sliders, CheckSquare, X, Briefcase, Video, Shield, Check,
    } = window.Icons || {};
        ...
        <header className={`h-14 flex items-center justify-between px-4 md:px-5 z-30 sticky top-0 backdrop-blur-md border-b transition-colors flex-shrink-0 ${isDarkMode ? 'bg-slate-900/90 border-slate-800' : 'bg-white/90 border-slate-100'}`}>
            ...
            <div className="flex items-center gap-2 md:gap-3">
                <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border transition cursor-pointer ${isDarkMode ? 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700' : 'bg-slate-50 border-slate-200 text-slate-400 hover:bg-white hover:shadow-sm'}`} onClick={() => setShowGlobalSearch(true)}>
                    ...
                    <span className="text-xs hidden md:inline">TÃ¬m kiáº¿m <kbd className={`ml-1 text-[10px] font-bold px-1.5 py-0.5 rounded border ${isDarkMode ? 'border-slate-600 bg-slate-700' : 'border-slate-200 bg-white'}`}>âŒ˜K</kbd></span>
                </div>
                    ...
                    {showNotifications && (
                        <div className="fixed inset-0 z-[100] md:absolute md:inset-auto md:top-12 md:right-0 w-full md:w-96 md:rounded-2xl shadow-xl border overflow-hidden animate-fadeIn glass-panel bg-white/95 dark:bg-slate-900/95 border-slate-100 dark:border-slate-700 flex flex-col md:block" onClick={e => e.stopPropagation()}>
                            <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-white/50 dark:bg-slate-800/50">
                                <h3 className="font-bold text-sm text-slate-800 dark:text-white">ThÃ´ng bÃ¡o ({unreadCount})</h3>
                                <div className="flex items-center gap-2">
                                    <button className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-400" title="CÃ i Ä‘áº·t thÃ´ng bÃ¡o"><Sliders className="w-4 h-4" /></button>
                                    <button onClick={markAllAsRead} className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-400" title="ÄÃ¡nh dáº¥u táº¥t cáº£ lÃ  Ä‘Ã£ Ä‘á»c"><CheckSquare className="w-4 h-4" /></button>
                            ...
                            <div className="flex gap-1 px-4 py-2 border-b border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/30">
                                <button onClick={() => setNotifFilter('all')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition ${notifFilter === 'all' ? 'bg-white shadow-sm text-slate-800 dark:bg-slate-700 dark:text-white' : 'text-slate-400 hover:bg-white/50 dark:hover:bg-slate-700/50'}`}>Táº¥t cáº£</button>
                                <button onClick={() => setNotifFilter('unread')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition ${notifFilter === 'unread' ? 'bg-white shadow-sm text-blue-600 dark:bg-slate-700 dark:text-blue-400' : 'text-slate-400 hover:bg-white/50 dark:hover:bg-slate-700/50'}`}>ChÆ°a Ä‘á»c</button>
                                <button onClick={() => setNotifFilter('high')} className={`flex-1 py-1.5 text-xs font-bold rounded-lg transition ${notifFilter === 'high' ? 'bg-white shadow-sm text-red-600 dark:bg-slate-700 dark:text-red-400' : 'text-slate-400 hover:bg-white/50 dark:hover:bg-slate-700/50'}`}>Quan trá»ng</button>
                            </div>
                            ...
                            <div className="flex-1 overflow-y-auto hover-scroll p-2 max-h-[calc(100vh-140px)] md:max-h-[400px]">
                                {groupedNotifications.today && groupedNotifications.today.length > 0 && (
                                    <div className="mb-2">
                                        <h4 className="px-2 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider">HÃ´m nay</h4>
                                            ...
                                            <div key={n.id} className={`group p-3 rounded-xl mb-1 flex gap-3 hover:bg-slate-50 dark:hover:bg-slate-800 transition cursor-pointer relative overflow-hidden ${!n.read ? 'bg-blue-50/50 dark:bg-blue-900/10' : ''}`}>
                                                ...
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <div className="flex gap-2 mt-2">
                                                        {n.priority === 'high' && <span className="text-[9px] font-bold bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 px-1.5 py-0.5 rounded border border-red-200 dark:border-red-800/50">Quan trá»ng</span>}
                                                ...
                                                <div className={`flex flex-col gap-1 md:opacity-0 md:group-hover:opacity-100 opacity-100 transition-opacity justify-center ml-2 ${!n.read ? '' : 'hidden'}`}>
                                                    {!n.read && (
                                                        <button onClick={(e) => markAsRead(e, n.id)} className="p-1.5 rounded-full hover:bg-blue-100 text-blue-500" title="ÄÃ¡nh dáº¥u Ä‘Ã£ Ä‘á»c">
                                ...
                                {groupedNotifications.yesterday && groupedNotifications.yesterday.length > 0 && (
                                    <div>
                                        <h4 className="px-2 py-1 text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-2">TrÆ°á»›c Ä‘Ã³</h4>
                                ...
                                {notifications.length === 0 && <div className="p-8 text-center text-slate-400 text-xs">KhÃ´ng cÃ³ thÃ´ng bÃ¡o má»›i</div>}
                            </div>
                            <div className="p-2 border-t border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-900 mt-auto md:mt-0">
                                <button onClick={() => { setShowNotifications(false); if (window.__handleOpenApp) window.__handleOpenApp('social'); }} className="w-full py-2 text-xs font-bold text-blue-600 hover:bg-blue-100 dark:hover:bg-blue-900/30 rounded-lg transition">Xem táº¥t cáº£ thÃ´ng bÃ¡o</button>
                            </div>
                        </div>
                    )}
                </div>
