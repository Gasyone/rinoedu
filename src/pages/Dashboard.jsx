        ...
        <div className="space-y-6 animate-slide-up pb-24 md:pb-10 w-full animate-fadeIn">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6 w-full">
                <div className="md:col-span-8 rounded-3xl p-6 md:p-8 bg-white dark:bg-slate-800 shadow-card border border-slate-100 dark:border-slate-700 flex flex-col justify-center">
                    <h2 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-white">{getGreeting()}{greetingName}! ðŸ‘‹</h2>
                    <div className="mt-4">
                        <label className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2 block">TÃ¢m Ä‘iá»ƒm hÃ´m nay</label>
                        <div className="flex items-center gap-3 relative z-10 p-1">
                            ...
                            <input type="text" placeholder="Nháº­p má»¥c tiÃªu chÃ­nh cá»§a báº¡n..." value={dailyFocus} onChange={handleFocusChange} onKeyDown={handleFocusKeyDown} className="flex-1 bg-transparent text-lg md:text-xl text-slate-700 dark:text-slate-200 placeholder-slate-300 border-none outline-none focus:ring-0" />
                        </div>
                    </div>
                </div>
                <div className="md:col-span-4 rounded-3xl p-6 bg-gradient-to-br from-indigo-500 to-purple-600 text-white shadow-lg flex flex-col justify-between relative overflow-hidden group">
                    ...
                    <div>
                        <div className="flex justify-between items-start">
                            ...
                            <span className="text-xs font-medium bg-white/10 px-2 py-1 rounded">HÃ  Ná»™i</span>
                        </div>
                        <div className="mt-4">
                            <span className="text-4xl font-bold">28Â°C</span>
                            <p className="text-sm opacity-90">Náº¯ng nháº¹, thÃ­ch há»£p ra ngoÃ i.</p>
                        </div>
                    </div>
            ...
            <div className="w-full">
                <h3 className="text-sm font-bold text-slate-500 mb-3 px-1 uppercase tracking-wider">Truy cáº­p nhanh</h3>
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4 w-full">
                    ...
                    <div className="flex flex-col items-center justify-center gap-3 p-4 rounded-2xl border-2 border-dashed border-slate-200 dark:border-slate-700 hover:border-blue-400 hover:bg-blue-50 dark:hover:bg-slate-700 transition cursor-pointer text-slate-400 hover:text-blue-500 text-center">
            ...
            <div className="flex justify-between items-end mb-3 mt-8">
                <h3 className="text-sm font-bold text-slate-500 px-1 uppercase tracking-wider">KhÃ´ng gian lÃ m viá»‡c</h3>
                <button className="text-xs font-bold flex items-center gap-1 text-slate-400 hover:text-blue-600 transition">
                    <window.Icons.Settings className="w-3.5 h-3.5" /> TÃ¹y chá»‰nh
                </button>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 w-full">
                <div className="lg:col-span-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                    {[
                        { label: 'Sá»‘ dÆ° vÃ­', value: '15.000.000', sub: '+2.5% so vá»›i thÃ¡ng trÆ°á»›c', icon: CreditCard, color: 'text-violet-600', bg: 'bg-violet-50 dark:bg-violet-900/30' },
                        { label: 'Task cáº§n lÃ m', value: '04', sub: '2 task quÃ¡ háº¡n', icon: window.Icons.CheckSquare || CheckCircle, color: 'text-emerald-600', bg: 'bg-emerald-50 dark:bg-emerald-900/30' },
                        { label: 'Dá»± Ã¡n active', value: '03', sub: 'Deadline sáº¯p tá»›i: T5', icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50 dark:bg-blue-900/30' },
                    ].map((stat, idx) => (
                    ...
                    <div className="p-5 rounded-2xl bg-gradient-to-br from-orange-500 to-red-500 text-white shadow-sm hover:shadow-md transition-all flex flex-col justify-between h-32 relative overflow-hidden">
                        ...
                        <div className="relative z-10 flex justify-between items-end">
                            <div>
                                <p className="text-sm font-bold">Chá» phÃª duyá»‡t</p>
                                <p className="text-[10px] opacity-80 mt-0.5">Nghá»‰ phÃ©p, PO, Táº¡m á»©ng</p>
                            </div>
                            <button className="bg-white text-orange-600 text-xs font-bold px-3 py-1.5 rounded-lg hover:bg-orange-50 transition shadow-sm">Xá»­ lÃ½ ngay</button>
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-3xl bg-white dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shadow-card p-6 flex flex-col">
                    <h3 className="font-bold text-slate-800 dark:text-white mb-4 flex items-center gap-2"><Activity className="w-4 h-4 text-blue-500" /> Hoáº¡t Ä‘á»™ng cÃ¡ nhÃ¢n</h3>
                    <div className="space-y-4 relative flex-1 overflow-y-auto pr-2 custom-scrollbar">
                        ...
                        {[
                            { time: '10:30', title: 'ÄÃ£ duyá»‡t PO-2024-001', type: 'success' },
                            { time: '09:15', title: 'Meeting team Tech', type: 'info' },
                            { time: 'HÃ´m qua', title: 'Cáº­p nháº­t há»“ sÆ¡ nhÃ¢n sá»±', type: 'default' },
                            { time: 'HÃ´m qua', title: 'HoÃ n thÃ nh KhÃ³a há»c An toÃ n ATTT', type: 'success' },
                        ].map((act, i) => (
                    ...
                    <button className="mt-4 w-full py-2 text-xs font-bold text-emerald-500 hover:text-emerald-600 dark:text-emerald-400 dark:hover:text-emerald-300 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-xl transition shadow-sm">Xem táº¥t cáº£</button>
                </div>
            </div>
        </div >
    );
